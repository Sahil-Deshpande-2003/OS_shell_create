...
void add_to_history(const char *command) {
    printf("Inside add_to_history, command = %s\n",command);
    if (history_count < MAX_HISTORY_SIZE) {
        strcpy(history[history_count], command);
        printf("history[history_count] = %s\n",history[history_count]);
        history_count++;
    } else {
        // Shift all commands to make space for the new one
...
int main() {
    ...
    while (1) {
        ...
        char *temp_input = malloc(MAX_INPUT_SIZE);
        ...
        removeSpacesAndNewlines(temp_input);

        printf("input = %s\n",input);

        if (strcmp(temp_input, "history") == 0) {
            printf("INSIDE MUST IF\n");
            display_history();
            continue;
        }
